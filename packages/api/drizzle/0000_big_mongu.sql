...
ALTER TABLE "tenant_memberships" ADD CONSTRAINT "tenant_memberships_invited_by_user_id_user_id_fk" FOREIGN KEY ("invited_by_user_id") REFERENCES "public"."user"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "athlete_claim_links_token_hash_key" ON "athlete_claim_links" USING btree ("token_hash");--> statement-breakpoint
...
CREATE UNIQUE INDEX "athlete_user_accounts_user_key" ON "athlete_user_accounts" USING btree ("user_id");--> statement-breakpoint
...
CREATE UNIQUE INDEX "provider_activity_summaries_summary_key" ON "provider_activity_summaries" USING btree ("provider","athlete_id","summary_type","provider_summary_id");--> statement-breakpoint
...
CREATE UNIQUE INDEX "tenant_invitations_pending_key" ON "tenant_invitations" USING btree ("tenant_id","email") WHERE "tenant_invitations"."status" = 'pending';--> statement-breakpoint
...
CREATE UNIQUE INDEX "tenant_memberships_active_user_key" ON "tenant_memberships" USING btree ("user_id") WHERE "tenant_memberships"."status" = 'active';--> statement-breakpoint
