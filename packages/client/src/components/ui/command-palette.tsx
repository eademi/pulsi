  ...
  return (
    <Dialog.Root onOpenChange={onOpenChange} open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="surface-panel fixed inset-x-4 top-24 z-50 mx-auto flex w-full max-w-2xl flex-col rounded-panel p-4 outline-none">
          <Dialog.Title className="sr-only">Quick search</Dialog.Title>
          <div className="input-shell h-12 rounded-soft border border-white/10">
          ...
          <div className="mt-3 grid gap-1">
            {items.map((item) => (
              <Link
                className="flex items-center justify-between rounded-soft px-3 py-3 transition hover:bg-accent-500/10"
                key={item.href}
